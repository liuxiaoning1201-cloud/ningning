/**
 * 題卡產生器（前端鏡像）— 用於本地對戰的 QA 模式。
 * 與 workers/zizi-zhuji/src/questionDeck.ts 保持一致。
 */
...
const NEAR_FORM: { correct: string; def: string; foils: string[] }[] = [
  ...
  { correct: "已", def: "已經的「已」", foils: ["己", "巳", "巴"] },
...
function buildIdiomMeaningCards(): QuestionCard[] {
  ...
  for (let i = 0; i < BUILTIN_IDIOMS.length; i++) {
    ...
    const wrongs = BUILTIN_IDIOMS
      .filter((it) => it.text !== target.text)
      .map((it) => it.definition);
    const picked = shuffle(wrongs).slice(0, 3);
    if (picked.length < 3) continue;
    const opts = shuffle([target.definition, ...picked]);
    cards.push({
      id: `im-${i}`,
      type: "idiom-meaning",
...
function buildComposeCards(): QuestionCard[] {
  return COMPOSE_SEEDS.map((s, i) => {
    const opts = shuffle([s.correct, ...s.foils]);
    return {
      id: `co-${i}`,
      type: "compose",
...
const POOLS: Map<number, { cards: QuestionCard[]; served: Set<string> }> = new Map();

function buildPool(difficulty: number): { cards: QuestionCard[]; served: Set<string> } {
  const cards: QuestionCard[] = [];
  if (difficulty <= 2) {
    ...
    cards.push(...buildNearFormCards());
    cards.push(...buildIdiomMeaningCards().slice(0, 40));
  } else {
...
export function drawQuestion(difficulty: number): QuestionCard {
  ...
  return { ...pick, id: `${pick.id}-${Date.now().toString(36)}` };
}
