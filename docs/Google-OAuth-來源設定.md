...
| 請加入的 URI | 說明 |
|-------------|------|
| `https://qingyiu.com` | **必加**。正式站（清沂遊），Google 登入在此來源直接執行 |
| `https://www.qingyiu.com` | 若使用 www（建議一併加） |
...
- 登入仍跳到 `zykongjian.pages.dev` → 代表瀏覽器還在用舊版 `auth-widget.js`，請強制重新整理（Cmd+Shift+R）
